...
from src.hyperbolicity.local import score_softmax, precompute_energies
...
def main():
    ...
    scores = np.array([score_softmax(n, dist_matrix, quad_energy, k, temperature) for n in tqdm(nodes, desc="Computing local scores")])
    elapsed = time.perf_counter() - t0
    print(f"[{method_name}] {k}-hop done in {elapsed:.2f}s")

    # --- Visualize ---
    save_path = os.path.join(save_dir, f"{k}_hop_{elapsed:.2f}s_{temperature}_temp.png")
