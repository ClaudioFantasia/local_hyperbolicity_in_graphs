...
from src.hyperbolicity.local import _get_quads_and_energies, score_KL_divergence, score_entropic
...
def compute_local_mu(target, dist_matrix, k, temperature, geometric_temperature, lambda_reg, method):
    ...
    if method == "KL_divergence":
        ...
        e_gromov_norm = normalize(e_gromov)
...
def compute_local_scores(nodes, dist_matrix, quad_cache, k, temperature, geometric_temperature, lambda_reg, method_name):
...
def main():
    ...
    parser.add_argument("--method", type=str, required=True, 
                        choices=["KL_divergence", "entropic"],
    ...
    quads, mu = compute_local_mu(target, dist_matrix, k, temperature, geometric_temperature, lambda_reg, method_name)
    node_contributions = compute_node_contributions(quads, mu, len(nodes))
    draw_graph_with_values(G, pos, node_contributions,
                           title=f"Mu Heatmap (target={target})")
    scores = compute_local_scores(nodes, dist_matrix, quad_cache, k, temperature, geometric_temperature, lambda_reg, method_name)
